    ...
    if _groq_client is None:
        ...
        if not api_key:
            raise EnvironmentError(
                "Missing GROQ_API_KEY. Set it in your environment or .env before running batch calls."
            )
...
class Batch:
    ...
    def send_file(self):
        ...
        with batch_file.open("rb") as f:
            response = get_groq_client().files.create(file=f, purpose="batch")
        self.file_id = response.id

    def submit_batch(self):
        response = get_groq_client().batches.create(
    ...
    def is_ready(self):
        response = get_groq_client().batches.retrieve(self.batch_id)
    ...
    def fetch_output(self):
        ...
        response = get_groq_client().files.content(self.output_file_id)
