...
def pip_install_pure_python(packages: list[str], package_dir: Path) -> list[str]:
    return [
        sys.executable,
        "-m",
        "pip",
...
def build_package() -> Path:
    ...
    build_dir = Path(tempfile.mkdtemp(prefix="planning-build-", dir=module_dir))
    package_dir = build_dir / "package"
    zip_path = module_dir / "lambda" / "planning_agent.zip"
    temp_zip_path = build_dir / "planning_agent.zip"
    requirements_file = project_root / "src" / "agents" / "PlanningAgent" / "requirements.lambda.txt"
    ...
    subprocess.check_call(
        pip_install_pure_python(["feedparser==6.0.12", "sgmllib3k==1.0.0"], package_dir)
    )
    ...
    files_to_copy = {
        ...
        project_root / "src" / "agents" / "planning_agent.py": package_dir / "src" / "agents" / "planning_agent.py",
        project_root / "src" / "agents" / "MessangingAgent" / "messaging_agent.py": package_dir / "src" / "agents" / "MessangingAgent" / "messaging_agent.py",
        project_root / "src" / "agents" / "ScannerAgent" / "deals.py": package_dir / "src" / "agents" / "ScannerAgent" / "deals.py",
        project_root / "src" / "agents" / "PlanningAgent" / "lambda_handler.py": package_dir / "src" / "agents" / "PlanningAgent" / "lambda_handler.py",
    }
